...
func TestShouldBypassTunnelConfiguredRules(t *testing.T) {
	...
	if !shouldBypassTunnel(N.NetworkTCP, M.Socksaddr{}, wfTCP) {
		t.Error("configured warframe TCP should bypass")
	}
	wfSrc := M.SocksaddrFrom(netip.MustParseAddr("10.0.0.5"), 4955)
	wfDst := M.SocksaddrFrom(netip.MustParseAddr("203.0.113.50"), 7777)
	if !shouldBypassTunnel(N.NetworkUDP, wfSrc, wfDst) {
		t.Error("warframe egress should bypass by source port")
	}
